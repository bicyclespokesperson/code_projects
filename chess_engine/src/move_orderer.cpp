...
{
  auto const previous_move_destination = board.get_previous_move().to;
  ...
  auto score = mvv_lva_table[m_victim_index][m_attacker_index];

  if (m.to == previous_move_destination)
  {
    score += 50;
  }
